...
def user_edit_post(user_id):
    ...
    with bc.form("Modifier message"):
        text = st.text_input("Modifier message", value=res["text"])
        submitted = st.form_submit_button("Submit")
        if submitted:
            pred, prob = predict(text)
            data = {
...
def admin_list_users():
    bc = st.beta_container()
    r = requests.get(API_PATH + f"/users/")
    for item in r.json():
...
def admin_add_user():
    ...
    with bc.form("Add user"):
        ...
        if submitted:
            ...
            r = requests.post(API_PATH + f"/users/", json=data)
    ...
    if user_id:
        ...
        with bc.form("Add user"):
            ...
            if submitted:
                ...
                r = requests.put(API_PATH + f"/users")
                bc.info(USER_EDITED)
    return bc
